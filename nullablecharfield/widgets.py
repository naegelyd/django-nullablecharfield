...
class NullableTextWidget(TextInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        if self.isnull:
            # Converted from django.utils.translation.string_concat from:
            #   https://docs.djangoproject.com/en/2.2/releases/1.11/#id2
            checkbox_strings = [
                ' <input type="checkbox" onchange="NullableText.null_event(this)" name="',
                widget_name,
                '" id="',
                widget_name,
                '" value="1"',
                checked,
                '><label class="vCheckboxLabel" for="',
                widget_name,
                '">',
                _('Not specified'),'</label>'
            ]
            checkbox = format_lazy('{}' * len(checkbox_strings), *checkbox_strings)

        # Converted from django.utils.translation.string_concat from:
        #   https://docs.djangoproject.com/en/2.2/releases/1.11/#id2
        strings = [
            super(TextInput, self).render(name, value, attrs),
            checkbox
        ]
